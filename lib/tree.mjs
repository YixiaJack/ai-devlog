...
export function buildTree(store) {
  ...
  for (const sid of order) {
    ...
    // Pass 1: build one idea node per real prompt, merging the AI work that
    // follows it into that node's `detail`. Placement comes after.
    const turnList = [];
    const byMsg = {};
    let turn = null;
    let acc = null;
    ...
    for (const m of msgs) {
      if (m.role === 'user' || m.role === 'system') {
        ...
        turn._msgId = m.id; turn._content = m.content;
        turnList.push(turn); byMsg[m.id] = turn;
        ...
        if (!turn) {
          ...
          turn._msgId = null; turn._content = '';
          turnList.push(turn);
          acc = { text: [], files: new Set(), diffs: [], commits: new Set(), endTs: m.timestamp };
        }
    ...
    // Pass 2: place the ideas into a hierarchy. If the LLM summarizer gave each
    // idea a parent, build that (deep, non-flat) tree; otherwise fall back to the
    // keyword heuristic.
    const summarized = turnList.some((t) => t._msgId && summaries[t._msgId]);
    if (summarized) placeWithSummaries(sessionNode, turnList, byMsg, summaries);
    else placeHeuristic(sessionNode, turnList);
    sessionNode.summary = `${countIdeas(sessionNode)} ideas`;
  }
...
// LLM-driven: each idea nests under the parent the model chose -> deep hierarchy.
function placeWithSummaries(sessionNode, turnList, byMsg, summaries) {
...
// Heuristic fallback: keyword intent + goal/refine/pivot nesting.
function placeHeuristic(sessionNode, turnList) {
  ...
  for (const t of turnList) {
    ...
    if (!threadRoot || intent === 'goal') { sessionNode.children.push(t); threadRoot = t; }
