...
function aiIdeaNodes(t, aiIdeas) {
  for (const ai of aiIdeas || []) {
    const en = (ai && ai.en) || (typeof ai === 'string' ? ai : '') || '';
    const zh = (ai && ai.zh) || (typeof ai === 'string' ? ai : '') || '';
    if (!en && !zh) continue;
    t.children.push(node('ai-idea', zh || en, {
      titleEn: en || zh, titleZh: zh || en,
...
// LLM-driven: one project-wide tree under a few CATEGORIES (classification
// first). A top-level idea (no parent) sits under its category; other ideas nest
// under the parent the model chose. Time order is preserved among siblings.
function placeGlobalSummaries(root, turnList, byMsg, summaries) {
  for (const t of turnList) {
    ...
    if (s) {
      const en = s.titleEn || s.ideaTitle || '';
      const zh = s.titleZh || s.ideaTitle || '';
      if (en || zh) { t.titleEn = en || zh; t.titleZh = zh || en; t.title = zh || en; }
      t.type = KIND2TYPE[s.kind] || 'idea';
      t.meta.intent = s.kind;
      t._catEn = s.categoryEn || s.category || 'General';
      t._catZh = s.categoryZh || s.categoryEn || s.category || '通用';
      t._parentMsg = s.parentId && byMsg[s.parentId] ? s.parentId : null;
      aiIdeaNodes(t, s.aiIdeas);
    } else {
      t.type = t.type || 'idea';
      t.titleEn = t.titleEn || t.title; t.titleZh = t.titleZh || t.title;
      t._catEn = 'General'; t._catZh = '通用';
      t._parentMsg = null;
    }
  }
  const cats = new Map();
  const catNode = (enName, zhName) => {
    const key = enName || 'General';
    if (!cats.has(key)) {
      const c = node('category', zhName || enName, { titleEn: enName, titleZh: zhName || enName, meta: {}, summary: '' });
      cats.set(key, c);
  ...
  for (const t of turnList) {
    ...
    if (cand && cand !== t && !ancestorContains(cand, t, byMsg)) cand.children.push(t);
    else catNode(t._catEn, t._catZh).children.push(t);
  }
  for (const c of cats.values()) c.summary = `${countTurns(c)} ideas`;
}

function countTurns(n) {
  let c = 0;
  (function w(x) { if (TURN_TYPE[x.type]) c++; (x.children || []).forEach(w); })(n);
  return c;
}
...
function placeGlobalHeuristic(root, turnList) {
  ...
  for (const t of turnList) {
    ...
    t.titleEn = t.title; t.titleZh = t.title;
